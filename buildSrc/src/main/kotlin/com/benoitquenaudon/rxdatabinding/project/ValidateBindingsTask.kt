    ...
    METHOD_ANNOTATIONS.forEach { annotation: String ->
      if (!annotationNames.contains(annotation)) {
        throw IllegalStateException(
            "Missing required @$annotation method annotation on ${(method.getEnclosingClass()).name}#${method.name}")
      }
    }
  }
  ...
  fun verifyParameters(method: MethodDeclaration) {
    ...
    params.forEach { p ->
      if (p.type is ReferenceType) {
        if (p.annotations == null || !p.annotations.map { it.name.toString() }.contains(
            "NonNull")) {
          throw IllegalStateException(
              "Missing required @NonNull annotation on ${method.getEnclosingClass().name}#${method.name} parameter: \"${p.id.name}\"")
        }
      }
    }
    ...
    if (params.size >= 2 && params[1].type is ReferenceType && params[1].coerceClassType().name == "Func1") {
      ...
      if (!func1Param.typeHasTypeArgs() || func1Param.coerceClassType().typeArgs[0] !is WildcardType) {
        throw IllegalStateException(
            "Missing wildcard type parameter declaration on ${method.getEnclosingClass().name}#${method.name} Func1 parameter: \"${func1Param.id.name}\"")
      }
    }
  }
  ...
  fun verifyReturnType(method: MethodDeclaration) {
    if (method.returnTypeAsType().name == "Action1") {
      if (!method.returnTypeHasTypeArgs() || method.returnTypeAsType().typeArgs[0] !is WildcardType) {
        throw IllegalStateException(
            "Missing wildcard type parameter declaration on ${method.getEnclosingClass().name}#${method.name}'s Action1 return type")
      }
    }
  }
